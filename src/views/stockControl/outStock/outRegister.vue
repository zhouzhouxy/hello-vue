        ...
        <el-row :gutter="24" class="header">
          ...
          <el-col :span="4">
            <el-select v-model="gather.reason" placeholder="入库理由">
              <el-option label="生产领料" value="1"></el-option>
              <el-option label="赠送" value="2"></el-option>
              <el-option label="内部借领" value="3"></el-option>
              <el-option label="其他借领" value="4"></el-option>
            </el-select>
          </el-col>
        </el-row>
      ...
      :visible.sync="dialogVisible2">
        <el-table :data="dFiles" stripe>
          ...
          <el-table-column  label="出库">
            <template slot-scope="scope">
                  <el-button @click="entry(scope.row)">出库</el-button>
              </template>
          </el-table-column>
        </el-table>
      <el-pagination
...
<script>
    export default {
      ...
      methods:{
          ...
          //确认提交
          commit(){
            var params={sPay:this.gather,payList:this.realDetails}
            console.log(params)
            //添加
            this.axios.put("http://127.0.0.1:1217/enxin/s-pay/add-s-pay",params,
