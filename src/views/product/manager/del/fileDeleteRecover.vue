  ...
  <div>
    ...
    <div v-show="!see">
      <el-table :data="list"
        ...
        <el-table-column label="恢复档案">
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="handleEdit(scope.$index, scope.row,list[scope.$index].id)">恢复档案</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
<script>
  export default {
    ...
    methods:{
      ...
      handleEdit(v1,v2,v3){
        //根据id逻辑删除
        this.axios.get("http://localhost:1217/enxin/d-file/recoverFile?id="+v3)
          .then((response)=>{
            //重新加载表
            this.loadTables();
          }).catch(function(error){
