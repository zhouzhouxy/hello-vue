  ...
  <div class="box">
    ...
    <el-dialog title="工序物料设计单"
      ...
      <el-table :data="procedureModule" >
        ...
        <el-table-column property="amountUnit" label="单位"></el-table-column>
        <el-table-column property="costPrice" label="单价(元)"></el-table-column>
        <el-table-column property="moduleSubtotal" label="小计(元)"></el-table-column>

      </el-table>
    </el-dialog>
    <el-dialog :visible.sync="dialogTableVisible3">
      <el-row :gutter="20">
        <span style="color: red">变更前将清除本工序物料设计，变更后需要对其他工序进行物料设计，您确认进行变更吗？
</span>
        <el-button-group>
  ...
  export default {
    ...
    methods:{
      ...
      confirm(){
        ...
        this.designProcedure.designModuleChangeTag="1";
        //提交
        this.designProcedure.changeTime=new Date();
      ...
      //确认重新设计
      confirmDel(){
        this.axios.get("http://localhost:1217/enxin/m-design-procedure-details/confirmUp?id="+this.pId)
          .then((response)=>{
            if(response.data.success){
              //隐藏
              this.dialogTableVisible3=false;
      ...
      loadTable(){
        this.axios.get("http://localhost:1217/enxin/m-design-procedure/queryProcedureById?id="+this.id)
          .then((response)=>{
            ...
            // var s=0;
            this.designProcedure.moduleCostPriceSum=0;
            for(let i=0;i<this.details.length;i++){
